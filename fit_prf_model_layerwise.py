...
def model_fitting(voxel_data, train_ids, val_ids, nest_ids, features_folder, args, device):
    ...
    num_features = clip_rn50_layer_size[args.layer_name][0]

    # num_features = 96
    num_voxels = train_voxel.shape[1]

    # best_weights: [num_features + 1, num_voxels] +1 for the intercept
    best_weights_array = np.zeros((num_features + 1, num_voxels)) 
    ...
    for prf_idx in tqdm(range(num_prfs)): # ~1450
        ...
        # train_features, nest_features, val_features = features[train_ids,:], features[nest_ids,:], features[val_ids,:]
        ...
        for voxel_idx in range(num_voxels):
            if best_nest_loss[voxel_idx] < best_nest_loss_array[voxel_idx]:
                best_weights_array[:,voxel_idx] = best_weights[:,voxel_idx].cpu().numpy()
    ...
    best_weights_array = best_weights_array
    ...
    return best_weights_array, best_lambda_array, best_nest_loss_array, best_prf_idx_array
...
def main():
    ...
    parser.add_argument('--layer_name', type=str, default="relu1")
    parser.add_argument('--save_root', type=str, default="/user_data/junruz/prf_models/svd_sklearn")
    ...
    best_weights, best_lambda, best_loss, best_prf_idx = model_fitting(voxel_data, train_ids, val_ids, nest_ids, features_folder, args, device)
    ...
    save_fits_roots = args.save_root
    save_fits_folder = os.path.join(save_fits_roots, f"S{ss}", args.model_name, args.layer_name)
    os.makedirs(save_fits_folder, exist_ok=True)
    # save the best weights, lambda, loss, and prf_idx
    np.save(os.path.join(save_fits_folder, "best_weights.npy"), best_weights)
