...
def downsample_and_flatten(features, size, layer_name):
    x = features[layer_name].float()  # [B, C, H, W]
...
def extract_intermediate_features(data_loader, args, num_imgs, device):
    """
    # Input: 
    ...
    for step, img_data in tqdm(enumerate(data_loader)):
            ...
            with torch.no_grad():
                ...
                feature = downsample_and_flatten(feature, target_sizes[args.layer_name], args.layer_name)
...
def save_features(features, args, dtype=np.float32):
    ...
    print(f"Saving features, shape: {features.shape}")
    features = features.astype(dtype)

    np.save(os.path.join(output_folder, f'features.npy'), features)
...
def main():
    ...
    parser.add_argument('--save_root', type=str, default="/user_data/junruz/prf_features/downsampled")
    ...
    neural_train_loader = torch.utils.data.DataLoader(loader, \
                                                ...
                                                num_workers=0, \
                                                drop_last=False)
    print(f"Using {args.layer_name} of {args.model_name}")
    features = extract_intermediate_features(neural_train_loader, args, num_images, device)
    print(f"Features shape: {features.shape}")
    save_features(features, args)
