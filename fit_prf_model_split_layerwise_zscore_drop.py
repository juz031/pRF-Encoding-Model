...
def model_fitting(voxel_data, train_ids, val_ids, nest_ids, features_folder, args, device):
    ...
    best_features_s_dict = {}
    best_features_m_dict = {}
    ...
    for prf_idx in tqdm(range(num_prfs)): # ~1450
        ...
        train_features, val_features, nest_features, keep_idx, features_s, features_m = model_fitting_utils.split_normalize_feats_with_drop(features, train_ids, val_ids, nest_ids)
        ...
        for voxel_idx in range(num_voxels):
            if best_nest_loss[voxel_idx] < best_nest_loss_array[voxel_idx]:
                ...
                best_features_s_dict[voxel_idx] = features_s
                best_features_m_dict[voxel_idx] = features_m
                count += 1
    ...
    return best_weights_dict, channel_kept_dict, best_lambda_array, best_nest_loss_array, best_prf_idx_array, best_features_s_dict, best_features_m_dict
...
def main():
    ...
    parser.add_argument('--split_root', type=str, default="/user_data/junruz/prf_models/split_1_zscore_drop")
    ...
    best_weights, channel_kept, best_lambda, best_loss, best_prf_idx, best_features_s, best_features_m = model_fitting(voxel_data, train_ids, val_ids, nest_ids, features_folder, args, device)
    ...
    with open(os.path.join(save_fits_folder, "best_features_s.pkl"), "wb") as f:
        pickle.dump(best_features_s, f)
    with open(os.path.join(save_fits_folder, "best_features_m.pkl"), "wb") as f:
        pickle.dump(best_features_m, f)
