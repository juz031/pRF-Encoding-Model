...
def fit_model(args):
    ...
    features_subfolder = os.path.join(features_folder, '%s_prf_%s'%(args.model_name, grid_name), 'NSD_S%d'%args.subject)

    if not os.path.exists(features_subfolder):
        features_subfolder = os.path.join(features_folder, 'old', '%s_prf_%s'%(args.model_name, grid_name), 'NSD_S%d'%args.subject)
        if not os.path.exists(features_subfolder):
            raise FileNotFoundError('Folder does not exist: %s'%features_subfolder)
    
    # load the first set of features, just to get size
    features_filename = os.path.join(features_subfolder, 'features_prf_0.npy')
    f = np.load(features_filename)

    print(f.shape)
    
    # def get_unique(f):
    #     # Keep just the unique columns in f.
    #     _, unique_indices = np.unique(f, axis=1, return_index=True)
    #     unique_indices_sorted = np.sort(unique_indices)
    #     f_unique = f[:, unique_indices_sorted]
    #     return f_unique
    ...
    r2_all = np.zeros((n_voxels, n_prfs))
    corr_all = np.zeros((n_voxels, n_prfs))
    weights_all = np.zeros((n_voxels, n_feat+1, n_prfs)) # n_feat + 1, for the intercept. last weight is intercept.
    featsens_all = np.zeros((n_voxels, n_feat, n_prfs)) # feature sensitivity values 
    best_lambda_inds_all = np.zeros((n_voxels, n_prfs))

    nest_loss_all = np.zeros((n_voxels, n_prfs)) # this is needed to choose the best pRF for each voxel
    ...
    for pi in np.arange(n_prfs):
        ...
        features_filename = os.path.join(features_subfolder, 'features_prf_%d.npy'%pi)
        print('loading features from: %s'%features_filename)
        sys.stdout.flush()
        f = np.load(features_filename)
        ...
        # put values back into my big array
        r2_all[:,pi] = r2 # good
        corr_all[:,pi] = corr # good
        weights_all[:,:,pi] = weights.T
        featsens_all[:,:,pi] = featsens
        best_lambda_inds_all[:,pi] = best_lambda_inds
    ...
    nest_loss_best = np.zeros((n_voxels, ))
    ...
    for vi in np.arange(0, n_voxels):
        ...
        nest_loss_best[vi] = nest_loss_all[vi, best_prf_ind]
        
        voxel_prf_grid_inds[vi] = best_prf_ind

        # only want to keep the param corresponding to this best pRF.
        r2_best[vi] = r2_all[vi,best_prf_ind] # this is val set R2, not nested set
        corr_best[vi] = corr_all[vi,best_prf_ind]
        weights_best[vi,:] = weights_all[vi,:,best_prf_ind]
        featsens_best[vi,:] = featsens_all[vi,:,best_prf_ind]
        best_lambda_inds_best[vi] = best_lambda_inds_all[vi,best_prf_ind]
    ...
    save_folder = os.path.join(save_fits_folder, '%s_fit_prfs'%(args.model_name))
    if not os.path.exists(save_folder):
        os.makedirs(save_folder)

    fn2save = os.path.join(save_folder, 'NSD_S%d_%s_fit_prfs.npy'%(args.subject, args.model_name))
