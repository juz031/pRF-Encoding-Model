...
def eval_prf_model_layerwise(voxel_data, train_ids, val_ids, nest_ids, features_folder, args, device):
    model_path = os.path.join(args.model_path, args.layer_name)
    weights_path = os.path.join(model_path, 'best_weights.npy')
    ...
    assert(num_voxels == best_weights.shape[1])
    ...
    #         pass
        ...
        weights = best_weights[:-1, voxel_idx]
        assert(weights.shape[0] == num_layer_feature)
        intercept = best_weights[-1, voxel_idx]
        ...
        train_features, val_features, nest_features = model_fitting_utils.split_normalize_feats(features, train_ids, val_ids, nest_ids)
        ...
        # if np.abs(predicted_neural).max() > 100:
        #     print(f"predicted neural is too large: {np.abs(predicted_neural).max()}")
        #     print(f"voxel index: {voxel_idx}")
        #     print(f"indices: {np.where(np.abs(predicted_neural) > 100)}")
...
def calculate_r2(predicted_neural, true_neural, roi_masks, noise_ceiling, args):
    ...
    for area in areas:
        ...
        save_root = f'/home/junruz/prf_model/figure/svd_eps1e-4/S{args.subject_id[0]}/{area}'
        ...
        save_folder = os.path.join(save_root, 'r2_hist')
        # plt.xlim(0,1)
        os.makedirs(save_folder, exist_ok=True)
        save_path = os.path.join(save_folder, f'{area}_{args.layer_name}_nc_{noise_ceiling_threshold}_r2.png')
        ...
        # plt.ylim(-0.3, 0.8)
        # plt.xlim(0,1)
        plt.title(f'r2 vs nc for {area} {args.layer_name}, nc > {noise_ceiling_threshold}')
        plt.axline((0, 0), slope=1, color='k', linestyle='--')
        save_folder = os.path.join(save_root, 'r2_nc')
        os.makedirs(save_folder, exist_ok=True)
        save_path = os.path.join(save_folder, f'{area}_{args.layer_name}_nc_{noise_ceiling_threshold}_r2_nc.png')
        plt.savefig(save_path, dpi=300)
        plt.close()
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='/user_data/junruz/prf_models/svd/S1/RN50')
    ...
    with open(os.path.join(args.model_path, args.layer_name, "r2.pkl"), "wb") as f:
        pickle.dump(r2, f)
