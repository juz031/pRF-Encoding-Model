    ...
    for prf_idx in tqdm(range(num_prfs)): # ~1450
        ...
        for voxel_idx in range(num_voxels):
            if best_nest_loss[voxel_idx] < best_nest_loss_array[voxel_idx]:
                ...
                best_features_s_array[voxel_idx] = features_s
                best_features_m_array[voxel_idx] = features_m
                count += 1
...
def main():
    ...
    best_weights, channel_ignored, best_lambda, best_loss, best_prf_idx, best_features_s, best_features_m = model_fitting(voxel_data, train_ids, val_ids, nest_ids, features_folder, args, device)
