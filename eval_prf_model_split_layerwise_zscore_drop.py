...
def eval_prf_model_layerwise(voxel_data, train_ids, val_ids, nest_ids, features_folder, args, device):
    ...
    features_m_path = os.path.join(model_path, 'best_features_m.pkl')
    features_s_path = os.path.join(model_path, 'best_features_s.pkl')
    
    with open(weights_path, "rb") as f:
        best_weights = pickle.load(f)
    with open(channel_kept_path, "rb") as f:
        channel_kept_dict = pickle.load(f)
    best_prf_idx = np.load(prf_idx_path)
    best_lambda = np.load(lambda_path)
    with open(features_m_path, "rb") as f:
        best_features_m = pickle.load(f)
    with open(features_s_path, "rb") as f:
        best_features_s = pickle.load(f)
    ...
    #         pass
        ...
        channel_kept = channel_kept_dict[str(voxel_idx)]
        ...
        features = features[:, channel_kept]
...
def calculate_r2(predicted_neural, true_neural, roi_masks, noise_ceiling, args):
    ...
    for area in areas:
        ...
        save_path = os.path.join(save_root, f'hist_{area}_{args.layer_name}_nc_{noise_ceiling_threshold}.pdf')
        ...
        plt.ylabel('$R^2$ per voxel')
        plt.ylim(-0.2,0.8)
        # plt.xlim(0,1)
        plt.title(f'$R^2$ vs NC for {area}, ResNet50 {args.layer_name}') #, nc > {noise_ceiling_threshold
        mean_r2 = np.mean(r2_voxels)
        median_r2 = np.median(r2_voxels)
        textstr = f"Mean: {mean_r2:.3f}\nMedian: {median_r2:.3f}"
        props = dict(boxstyle='round', facecolor='white', alpha=0.75)
        ...
        # os.makedirs(save_folder, exist_ok=True)
        save_path = os.path.join(save_root, f'scatter_{area}_{args.layer_name}_nc_{noise_ceiling_threshold}.pdf')
...
def main():
    ...
    parser.add_argument('--model_path', type=str, default='/user_data/junruz/prf_models/split_1_zscore_drop/S1')
    ...
    parser.add_argument('--model_name', type=str, default="OPEN_CLIP_RN50")
